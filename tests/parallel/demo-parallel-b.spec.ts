...
test.only('test B', async ({ page }, testInfo) => {
  console.log(`test A parallelIndex - ${testInfo.parallelIndex}`);
  await page.goto('https://demoqa.com/login');
  
  await page.fill('id=userName', 'demoqahive0002');
  ...
  await expect(page.locator('id=userName-value')).toContainText('demoqahive0002');
});
